...
const TaskCard = ({ task, onToggle }: TaskCardProps) => {
  ...
  return (
    <div
      className={`relative p-4 rounded-lg border ${isCancelled ? 'bg-gray-50' : "bg-[#fff8ec]"} shadow-sm transition-all duration-300 flex flex-col justify-between ${isEmergency ? 'border-red-700 border-2' : isCancelled ? "border-gray-300" : 'border-yellow-700'
    ...
    >
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          {isEmergency && (
            <span className="text-sm text-white font-bold px-2 py-1 bg-red-600 rounded">Urgente</span>
          )}
          {isCancelled && (
            <span className="text-sm text-white font-bold rounded px-2 py-1 bg-gray-400">Cancelado</span>
          )}
        </div>
